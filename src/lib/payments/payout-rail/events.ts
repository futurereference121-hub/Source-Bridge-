...
/** GET full outbound payment for fee/FX snapshot (thin events lack body). */
async function retrieveOutboundPaymentBody(
...
async function reconcileOutboundFromEvent(opts: {
  ...
  if (mapped === "SUCCEEDED" && attempt.status !== "SUCCEEDED") {
    ...
    // G7: retrieve full outbound for fee/FX snapshot when thin event has no body.
    const providerBody = await retrieveOutboundPaymentBody(
  ...
  if (mapped === "FAILED" || mapped === "ACTION_REQUIRED") {
    await prisma.outboundPaymentAttempt.updateMany({
      where: {
        id: attempt.id,
        status: { notIn: ["SUCCEEDED", "RECONCILED", "RETURNED"] },
      },
      data: {
        status: mapped,
        failureCode: mapped,
        failureMessage: sanitizeProviderFailureText(mapped),
  ...
  if (mapped === "RETURNED") {
    // G2: reverse books if previously SUCCEEDED; block auto-repay.
    await handleOutboundReturned({
      attemptId: attempt.id,
      outboundId: opts.outboundPaymentId,
      failureCode: "RETURNED",
      failureMessage: "Provider returned outbound payment",
    });
    return;
  }
